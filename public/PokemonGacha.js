export class PokemonGacha {
    ...
    async showPokemon(){
        const pageElement = document.getElementById("page")
        const createPokemonWindow = document.createElement('div')
        createPokemonWindow.id = "pokemonWindow"
        pageElement.appendChild(createPokemonWindow)
        const pokemonWindow = document.getElementById("pokemonWindow")
        pokemonWindow.appendChild(await this.makePokemonImg())
    ...
    async randomSelectPokemon(){
        ...
        const pokemonNum = Math.floor(Math.random()*1000)
        const result = await fetch(`https://pokeapi.co/api/v2/pokemon/${pokemonNum}/`);
        this.pokemon = await result.json();
    }
